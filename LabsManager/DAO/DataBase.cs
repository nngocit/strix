...
namespace LabsManager.DAO
{
    class DataBase
    {
        ...
        // private SqlConnection cnn1;
        public ReadConnect constring;
        public DataBase()
        {
        }
        public DataBase(string connecstring, bool t)
        {

            //cnn1 = new SqlConnection(connecstring);
        ...
        public DataBase(string path)
        {
            try
            {
                ...
                constring = new ReadConnect(false, path);
                if (constring.GetConnectionWithinifile())
                {
                    string connectring = constring.ConnectionString;
                ...
                else
                {
                    ...
                    MessageBox.Show("Không có đọc được chuỗi kết nối \nHay chuỗi kết nối không tồn tại. Hãy thiết lập chuỗi kết nối", "Thông báo", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
        ...
        public DataBase(string path, ketnoi _ketnoi)
        {
            try
            {
                ...
                {
                    string connectring = constring.ConnectionString;
                    _ketnoi.ServerName = constring.ServerName;
                    _ketnoi.DatabaseName = constring.DatabaseName;
                    _ketnoi.UserName = constring.UserName;
                    _ketnoi.PassWord = constring.PassWord;
                    cnn = new SqlConnection(connectring);
                    cmd = cnn.CreateCommand();
                }
                else
        ...
        public bool MyExcuteNonQuery(string sql, CommandType ct, ref string err, params SqlParameter[] param)
        {
            bool ketqua = false;
            try
            {
